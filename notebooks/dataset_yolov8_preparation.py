...
def write_readme(path: Path) -> None:
    """Writes the README.md file of the dataset that describes how to train a YOLOv8
    model on it."""
    content = [
        "# README",
        "",
...
def get_X(
    dataset_names: list[str], invalid_seaview_quadratids: set[Quadratid]
) -> list[Entry]:
    """Returns a list of {dataset_name, image_filepath, label_filepath} that constitues
    the X dataset.
    ...
    for dataset_name in dataset_names:
        ...
        if len(all_image_filepaths) > len(image_filepaths):
            print(
                ...
                f" from {dataset_name} because of label mismatch"
            )
...
def split_train_val(
    ...
    """Returns a splitted dataset X into X_train and X_val using the `train_size_ratio`
    and the random `seed`."""
    N = len(X)
    random.seed(seed)
    random.shuffle(X)
...
def write_entry(
    ...
    """Given an `entry` and a mode in #{`train`, `val`}, it writes it in a YOLOv8
    format."""
...
def write_dataset(
    ...
    """Writes the dataset splitted in X_train and X_val into the right folder structure
    for YOLOv8."""
